...
| <a name="input_cloudwatch_metrics_role_name"></a> [cloudwatch\_metrics\_role\_name](#input\_cloudwatch\_metrics\_role\_name) | Name for the IAM role used by the CloudWatch Metric Stream. If null, Terraform generates a name using var.name as a prefix. Changing this on an existing deployment replaces the IAM role. | `string` | `null` | no |
| <a name="input_cloudwatch_metrics_role_permissions_boundary"></a> [cloudwatch\_metrics\_role\_permissions\_boundary](#input\_cloudwatch\_metrics\_role\_permissions\_boundary) | ARN of the permissions boundary policy to attach to the CloudWatch Metric Stream IAM role. | `string` | `null` | no |
