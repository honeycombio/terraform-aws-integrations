...
| <a name="input_lambda_role_name"></a> [lambda\_role\_name](#input\_lambda\_role\_name) | Name for the IAM role used by the RDS log transform Lambda function. If null, the Lambda function name is used. Changing this on an existing deployment replaces the IAM role. | `string` | `null` | no |
