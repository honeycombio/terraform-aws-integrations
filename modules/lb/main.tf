...
module "s3_processor" {
  ...
  s3_existing_package = {
    bucket = compact([
      var.lambda_package_bucket,
      "honeycomb-integrations-${data.aws_region.current.name}",
    ])[0]
    key = compact([
      var.lambda_package_key,
      "agentless-integrations-for-aws/LATEST/ingest-handlers.zip",
    ])[0]
  }
